...
function _pipeModels(mainDirectory, searchDirectory, models) {
    ...
    const relativePath = Path.relative(mainDirectory, searchDirectory);

    let isParentModelFolder = relativePath.split(Path.sep).includes("models") || relativePath.split(Path.sep).includes("model");
