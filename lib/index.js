...
/**
 *
 * @params modelDirectory
 * @return {{}}
 */
function pipeModels(modelDirectory) {

    try {
        const supposedModelDirectory = Fs.statSync(modelDirectory);
        if (!supposedModelDirectory.isDirectory()) {
            throw new Error("Given path is not a directory");
        }
    }
    catch (e) {
        console.log(Colors.error(e));
        console.log(Colors.error("Model folder not found in the given location: " + modelDirectory));
        return {};
    }

    const models = {};
    const modelFsItems = _readDirectoryRecursively(modelDirectory, _filterJavascriptFile);

    for (let modelFsItem of modelFsItems) {
        ...
        models[_getModelName(modelFsItem.name)] = model;
    }
 ...
 */
function pipeRoutes(server, hooks, controllerDirectory, routeFile) {
